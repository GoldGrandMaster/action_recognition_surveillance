...
COCO_PAIR = [(0, 13), (1, 2), (1, 3), (3, 5), (2, 4), (4, 6), (13, 7), (13, 8),  # Body  @yjy
             (7, 9), (8, 10), (9, 11), (10, 12)]
...
def draw_single(frame, pts, joint_format='coco'):
    ...
    part_line = {}
    pts = np.concatenate((pts, np.expand_dims((pts[1, :] + pts[2, :]) / 2, 0)), axis=0)  # 左肩、右肩求中点 @ yjy
    for n in range(pts.shape[0]):
        if pts[n, 2] <= 0.3:   # 可信度低于0.05跳过 @yjy
            continue
    ...
    for i, (start_p, end_p) in enumerate(l_pair):
        if start_p in part_line and end_p in part_line:
            ...
            cv2.line(frame, start_xy, end_xy, line_color[i], 2) # int(1*(pts[start_p, 2] + pts[end_p, 2]) + 1))
    return frame
...
def vis_frame(frame, im_res, joint_format='coco'):
    ...
    for human in im_res['result']:
        ...
        kp_preds = human['keypoints']
        kp_scores = human['kp_score']
        kp_preds = torch.cat((kp_preds, torch.unsqueeze((kp_preds[5, :]+kp_preds[6, :]) / 2, 0)))  # 加入一个左肩、右肩中心点 @yjy
        kp_scores = torch.cat((kp_scores, torch.unsqueeze((kp_scores[5, :]+kp_scores[6, :]) / 2, 0)))
        # Draw keypoints
        for n in range(kp_scores.shape[0]):
            if kp_scores[n] <= 0.05:   # 可信度小的跳过 @yjy original=0.05
                continue
            cor_x, cor_y = int(kp_preds[n, 0]), int(kp_preds[n, 1])
